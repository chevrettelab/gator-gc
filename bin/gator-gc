...
def generating_clustermap(concatenated_GFSs: str, output_dir: str) -> None:
    ...
    #ax1 = fig.add_axes([0.05,0.71,0.9,0.2])
    #dend1 = dendrogram(linkage_matrix, color_threshold=np.inf, orientation='top')
    #ax1.set_xticks([])
    #ax1.set_yticks([])
    ax2 = fig.add_axes([0.028,0.05,0.24,0.65])
    dend2 = dendrogram(linkage_matrix, link_color_func=lambda x: 'black', color_threshold=np.inf, orientation='left')
    ax2.set_xticks([])
    ax2.set_yticks([])
    ax2.axis('off')
    ax3 = fig.add_axes([0.27,0.05,0.68,0.65])
    cax = sns.heatmap(matrix_gcfs.iloc[dend2['leaves'], dend2['leaves']], ax=ax3, linewidths=1, linecolor='black', cmap='viridis', cbar_kws={'label': 'Gator Focal Score', 'shrink': 1}, xticklabels=False, cbar=False)
